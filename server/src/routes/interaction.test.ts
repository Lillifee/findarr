...
import * as interactionService from '../services/interaction.js';
import * as mediaService from '../services/media.js';
import { createMedia, mockDb, createUser } from '../utils/testHelper.js';
import { interactionRoutes, adminInteractionRoutes } from './interaction.js';
...
describe('interactionRoutes', () => {
  ...
  beforeEach(async () => {
    ...
    await app.register(adminInteractionRoutes, { prefix: '/interactions/admin' });
    await app.ready();
  });
  ...
  it('should return all interactions (admin)', async () => {
    const res = await app.inject({ method: 'GET', url: '/interactions/admin' });
