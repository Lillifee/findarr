...
import { mockDb, mockUserWithPassword } from '../utils/testHelper.js';
import { DUMMY_HASH, login, verifyPassword } from './auth.js';
...
describe('login', () => {
  ...
  it('should return user without password on valid credentials', async () => {
    vi.spyOn(userService, 'getUserByEmail').mockReturnValue(mockUserWithPassword);
    ...
    const { password_hash: _hash, ...expectedUser } = mockUserWithPassword;
